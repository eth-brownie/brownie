...
def test_new_account(capfd, monkeypatch):
    assert not DATA_FOLDER.joinpath("accounts/new.json").exists()
    cli_accounts._list()
    ...
    assert DATA_FOLDER.joinpath("accounts/new.json").exists()


def test_generate(capfd):
    path = DATA_FOLDER.joinpath("accounts/generate.json")
...
def test_import():
    path = DATA_FOLDER.joinpath("accounts/import-test.json")
    cli_accounts._generate("import-test")
    new_path = DATA_FOLDER.joinpath("x.json")
    path.rename(new_path)
    cli_accounts._import("import-new", str(new_path.absolute()))
    assert DATA_FOLDER.joinpath("accounts/import-new.json").exists()
...
def test_export():
    ...
    target_path = DATA_FOLDER.joinpath("exported.json")
...
def test_export_overwrite():
    ...
    path = str(DATA_FOLDER.joinpath("accounts/export-exists.json").absolute())
    with pytest.raises(FileExistsError):
...
def test_delete():
    ...
    path = DATA_FOLDER.joinpath("accounts/del-test.json")
    assert path.exists()
